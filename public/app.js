...
.when('/:suffix', {
  ...
  resolve: {
    userInfo($route, $http) {
      return $http.get('../api/hhrr/selection/' + $route.current.params.suffix).then(function (resp) {
  ...
  resolve: {
    userInfo($route, $http) {
      return $http.get('../api/hhrr/selection').then(function (resp) {
...
require('ui/modules')
.get('app/hhrr', [])
.controller('hhrr', function ($scope, $route, $location) {
