...
export default function (server, request, remoteUser, kibanaIndexSuffix, callback) {
  ...
  if (remoteUser == kibanaIndexSuffix.replace('%40', '@')) {
    ...
    server.log(['plugin:hhrr', 'debug'], 'kibanaIndexSuffix matches remote user name: ' + replacedKibanaIndexSuffix);
  ...
  if (config.get('hhrr.local.enabled')) {
    if (validateByLocal(server, kibanaIndexSuffix)) {
      server.log(['plugin:hhrr', 'debug'], 'kibanaIndexSuffix matches local group: ' + kibanaIndexSuffix);
