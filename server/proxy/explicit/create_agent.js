...
module.exports = _.memoize(function (server) {
  ...
  const agentOptions = {
    rejectUnauthorized: config.get('hhrr.explicit_kibana_index_url.kibana.ssl.verificationMode') == 'none' ? false : true
  };

  if (_.size(config.get('hhrr.explicit_kibana_index_url.kibana.ssl.certificateAuthorities'))) {
    agentOptions.ca = config.get('hhrr.explicit_kibana_index_url.kibana.ssl.certificateAuthorities').map(readFile);
  }
