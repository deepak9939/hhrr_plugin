...
module.exports = function(kbnServer, yarOptions) {
  ...
  server.register({
    ...
    if (err) {
      kbnServer.log(['plugin:hhrr', 'error'], 'Failed to load kibi-h2o2');
    }
  });

  const uri = url.parse(kbnServer.config().get('hhrr.explicit_kibana_index_url.proxy.url'));
  if (uri.protocol == 'https:') {
    server.connection({
      ...
      listener: httpolyglot.createServer({
        key: readFileSync(kbnServer.config().get('hhrr.explicit_kibana_index_url.proxy.ssl.key')),
        cert: readFileSync(kbnServer.config().get('hhrr.explicit_kibana_index_url.proxy.ssl.certificate')),
              ...
              if (originalPayload.length > 0) {
                ...
                if (replacedIndex) {
                  ...
                  if (modifiedPayload !== originalPayload) {
                    kbnServer.log(['plugin:hhrr', 'debug'], 'Replace the string in commons.bundle.js: "app/kibana" => "' + suffix + '/app/kibana"');
                  } else {
                    kbnServer.log(['plugin:hhrr', 'warning'], 'Failed to replace the string in commons.bundle.js: "app/kibana" => "' + suffix + '/app/kibana"');
                  }
                ...
                kbnServer.log(['plugin:hhrr', 'debug'], 'Replacement skipped because the target file has already been chached.');
                reply(response);
              }
            });
          } else {
  ...
  server.start((err) => {
    ...
    kbnServer.log(['plugin:hhrr', 'info'], 'Proxy server started at ' + server.info.uri);
  });
  ...
  server.register({
    ...
    if (err) {
      kbnServer.log(['plugin:hhrr', 'error'], 'Unknown error occured at the init()');
    }
  });
};
