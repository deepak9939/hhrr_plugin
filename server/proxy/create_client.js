...
module.exports = function (server) {
  ...
  class ElasticsearchClientLogging {
    error(err) {
      server.log(['plugin:hhrr', 'error'], err);
    }
    warning(message) {
      server.log(['plugin:hhrr', 'warning'], message);
    }
    info() {}
    debug() {}
    trace() {}
  ...
  function createClient(options) {
    options = _.defaults(options || {}, {
      url: config.get('hhrr.elasticsearch.url'),
      ...
      ca: config.get('hhrr.elasticsearch.ssl.certificateAuthorities'),
