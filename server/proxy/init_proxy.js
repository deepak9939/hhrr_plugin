...
module.exports = function(kbnServer) {
  ...
  if (uri.protocol == 'https:') {
    server.connection({
      ...
      listener: httpolyglot.createServer({
        key: readFileSync(kbnServer.config().get('hhrr.ssl.key')),
        cert: readFileSync(kbnServer.config().get('hhrr.ssl.certificate')),
      ...
      kibi_proxy: {
        ...
        onResponse: function (err, response, request, reply) {
          ...
          Wreck.read(response, { json: true }, function (err, payload) {
            if (kbnServer.config().get('hhrr.get_username_from_session.enabled')) {
              if (payload && payload.toString() == 'Unauthorized') {
                reply(Boom.unauthorized('plugin:hhrr: unauthorized'));
                return;
              }
            }
            ...
            if (replacedIndex && payload) {
              ...
              if (payload['error']) {
                ...
                if (payload['status'] === 409) {
                  reply(Boom.conflict('plugin:hhrr: document_already_exists_exception'));
                  return;
                }
  ...
  const yarOptions = {
    name: 'hhrr-session',
    cache: {
      expiresIn: kbnServer.config().get('hhrr.session.timeout')
    },
    cookieOptions: {
      password: kbnServer.config().get('hhrr.session.secretkey'),
      isSecure: kbnServer.config().get('hhrr.session.isSecure'),
      passThrough: true
    }
  };
  ...
  if (kbnServer.config().get('hhrr.explicit_kibana_index_url.enabled')) {
