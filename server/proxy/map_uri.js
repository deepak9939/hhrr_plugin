...
export default function mapUri(server) {
  ...
  return function (request, done) {
    const {
      ...
      query: esUrlQuery
    } = parseUrl(config.get('hhrr.elasticsearch.url'), true);
    ...
    function coreMapUri(path, kibanaIndexAction) {
      ...
      if (kibanaIndexAction) {
        server.log(['plugin:hhrr', 'debug'], 'mappedUrl: ' + mappedUrl);
        ...
        if (config.get('hhrr.force_to_access_by_es_user')) {
          ...
          if (username && password) {
            ...
            server.log(['plugin:hhrr', 'debug'], 'authorization header has been overridden.');
          }
        }
        ...
        if (kibanaIndexAction == 'created') {
          ...
          sleep(config.get('hhrr.wait_kibana_index_creation')).then(function () {
            server.log(['plugin:hhrr', 'debug'], 'Wait a few seconds for Kibana index creation');
    ...
    if (replacedIndex && path.indexOf(originalIndex) > -1 && path.indexOf(replacedIndex) === -1) {
      ...
      server.log(['plugin:hhrr', 'debug'], 'Replace kibana.index "' + originalIndex + '" with "' + replacedIndex + '"');
      server.log(['plugin:hhrr', 'debug'], 'Replaced path: ' + replacedPath);
