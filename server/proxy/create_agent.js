...
module.exports = _.memoize(function (server) {
  ...
  const target = url.parse(config.get('hhrr.elasticsearch.url'));
  ...
  if (_.size(config.get('hhrr.elasticsearch.ssl.certificateAuthorities'))) {
    agentOptions.ca = config.get('hhrr.elasticsearch.ssl.certificateAuthorities').map(readFile);
  }
