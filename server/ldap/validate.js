...
export default function (server, request, remoteUser, kibanaIndexSuffix, callback) {
  ...
  function searchGroups(error, response) {
    if (error != undefined) {
      server.log(['plugin:hhrr', 'error'], 'LDAP search error: ' + error);
      return (callback === null) ? false : getGroups(server, request, remoteUser, callback);
    }

    response.on('searchEntry', function(entry) {
      server.log(['plugin:hhrr', 'debug'], 'LDAP search result: ' + entry.object[ldapConfig.rolenameAttribute]);
    ...
    response.on('error', function(error) {
      server.log(['plugin:hhrr', 'error'], 'LDAP search error: ' + error.message);
      return (callback === null) ? false : getGroups(server, request, remoteUser, callback);
    });

    response.on('end', function(result) {
      server.log(['plugin:hhrr', 'debug'], 'LDAP search status: ' + result.status);
      server.log(['plugin:hhrr', 'debug'], 'Found LDAP groups: ' + groups.toString());
      for (let i = 0; i < groups.length; i++) {
        if (groups[i] == kibanaIndexSuffix) {
          server.log(['plugin:hhrr', 'debug'], 'kibanaIndexSuffix matches LDAP group: ' + kibanaIndexSuffix);
  ...
  if (server.config().get('hhrr.ldap.get_dn_dynamically') && server.config().get('hhrr.ldap.member_attribute') != 'memberUid') {
    const userbase = server.config().get('hhrr.ldap.userbase');
    const usernameAttribute = server.config().get('hhrr.ldap.username_attribute');
    const options = {
      scope: 'sub',
    ...
    ldapConfig.client.search(userbase, options, function (error, response) {
      ...
      response.on('searchEntry', function(entry) {
        server.log(['plugin:hhrr', 'debug'], 'LDAP search result: ' + entry.object['dn']);
        if (entry.object['dn']) {
          dn = entry.object['dn'];
        }
      });
      ...
      response.on('end', function(result) {
        server.log(['plugin:hhrr', 'debug'], 'LDAP search status: ' + result.status);
        server.log(['plugin:hhrr', 'debug'], 'Found DN: ' + dn);
